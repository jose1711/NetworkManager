...
{
	gs_free const NMTeamLinkWatcher *const*a_free = NULL;
	gs_free const NMTeamLinkWatcher *const*b_free = NULL;
	guint i;
	...
	if (   ignore_order
		...
		g_qsort_with_data (a, len, sizeof (*a), _team_link_watchers_cmp_p_with_data, NULL);
		g_qsort_with_data (b, len, sizeof (*b), _team_link_watchers_cmp_p_with_data, NULL);
	}
	for (i = 0; i < len; i++) {
...
NM_GOBJECT_PROPERTIES_DEFINE_NOTIFY (NMSettingTeam, obj_properties, NMTeamAttribute, _NM_TEAM_ATTRIBUTE_0);
                                  ...
                                  NMTeamLinkWatcher *link_watcher)
{
	...
	return _maybe_changed (setting,
	                       nm_team_setting_value_link_watchers_add_dup (NM_SETTING_TEAM_GET_PRIVATE (setting)->team_setting,
	                                                                    link_watcher));
}
              ...
              GValue *value, GParamSpec *pspec)
{
	...
	case NM_TEAM_ATTRIBUTE_LINK_WATCHERS:
		v_ptrarr = priv->team_setting->d.link_watchers;
		g_value_take_boxed (value, _nm_utils_copy_array (v_ptrarr,
